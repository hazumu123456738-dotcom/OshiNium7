...
enum SubscriptionLimits {
    ...
    // ★ コミュニティカレンダーの予定作成の1日あたり上限(荒らし対策)。DMの新規スレッド上限とは
    //   異なり、予定追加はグループ全員のチャットにシステムメッセージ・承認依頼通知を飛ばすため、
    //   被害を受けるのは送信者本人ではなく他のグループメンバー側になる。そのため
    //   プレミアム会員でも無制限にはせず、上限を緩めるだけに留める
    static func eventCreateDailyLimit(isPremium: Bool) -> Int {
        isPremium ? 40 : 20
    }
