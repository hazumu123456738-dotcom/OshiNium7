//
//  AnonymousTopicSearchView.swift
//  OshiNium7
//
...
// ★ 匿名トークルーム一覧右上の虫眼鏡から開く検索。「自分が知りたいことを話している人が
//   いないか」をトークルームのタイトルから探せるようにする。トークルームは既に一覧画面の
//   リスナーで全件ロード済みのため、Firestore側の全文検索は不要でクライアント側フィルタで足りる
struct AnonymousTopicSearchView: View {
    ...
    private var results: [AnonymousTopic] {
        let trimmed = query.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return [] }
        return chatViewModel.anonymousTopics.filter {
    ...
    private func resultRow(_ topic: AnonymousTopic) -> some View {
        HStack(spacing: 12) {
            Circle()
