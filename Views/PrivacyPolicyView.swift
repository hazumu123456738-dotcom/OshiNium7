        ...
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 20) {
                ...
                LegalSection(title: "1. 取得する情報") {
                    ...
                    LegalBullet("通知：イベントの予定や持ち物チェックリストのリマインダーは端末内でスケジュールされるローカル通知です。一方、フォロー・いいね・コメント等の通知は、Firebase Cloud Messaging（Google LLC）を経由したプッシュ通知として配信されるため、通知のタイトル・本文がFirebaseのサーバーを経由します。")
                ...
                LegalSection(title: "6. データの保管・削除") {
                    Text("取得した情報は、本アプリの提供に必要な期間保管します。アカウントの削除は、アプリ内の「マイページ」からご自身でいつでも行うことができます。削除を実行すると、ログイン情報（アカウント本体）はその場で削除され、以後は再ログインができなくなります。プロフィール・投稿・コメント・フォロー関係・通知・カレンダーや持ち物チェックリスト等の推し活記録、アップロード済みの画像・動画についても、削除の直後に自動的な消去処理が始まります（サーバー側の処理のため、完全に消えるまで数分程度かかる場合があります）。")
                    Text("一方で、以下の情報は、他のユーザーの利用体験やコミュニティの安全を守るため、アカウント削除後も削除されずに残ります。")
                        .font(.system(size: 13))
                        .foregroundColor(.secondary)
                    LegalBullet("グループチャット・ダイレクトメッセージであなた自身が送信したメッセージ本文：会話の相手やグループメンバーの手元に残る会話履歴を壊さないよう、送信済みのメッセージはそのまま残ります（送信者の表示名は削除済みである旨がわかる表示に変わります）。")
                    LegalBullet("通報に関する記録（あなたが行った通報、あなたが対象になった通報）：不正利用の防止と、深刻なケースでの法的対応に備えるための記録として保持します。")
                }
...
//   プライバシーポリシーと利用規約で改定日が異なるため別々に持つ
//   （HTML版のlegal/privacy-policy.html・legal/terms-of-service.htmlの
//   最終更新日と必ず一致させること）
enum LegalDocumentContent {
    static let lastUpdated = "2026年8月18日"
    static let termsLastUpdated = "2026年8月16日"
}
